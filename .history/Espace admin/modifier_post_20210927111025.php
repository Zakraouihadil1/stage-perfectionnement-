...
/*cette requètte préparée sert à modifier les données j'ai rajouté "nouveau" et "nouvelle" au nom des variables mais vous n'êtes pas obligé des faire pareil, c'est juste pour mieux me repérer et comprendre ce que je fais.*/
	...
	$req = $bdd->prepare('UPDATE masterlogin set username= :nouveau_username , email= :nouveau_email, password=:nouveau_password,role=:nouveau_role WHERE id=:id"');
    $req->bindParam(':nouveau_username',$username);
    $req->bindParam(':nouveau_email',$email);
    $req->bindParam(':nouveau_password',$password);
    $req->bindParam('nouveau_role',$role);
	$req->execute(array( 
